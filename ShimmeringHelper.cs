...
{
    private readonly WeakReference<FrameworkElement> _associatedObject;
    public FrameworkElement AssociatedObject
    {
        get
        {
            _associatedObject.TryGetTarget(out var result);
            return result;
        }
    }
    ...
    {
        _associatedObject = new(associatedObject);
    ...
    private void CheckMaskBrushSource()
    {
        ...
        AddColorListeners();
        UpdateMaskBrushSource(GetBrushForColor(compositor, CalculateAlternativeColor(AssociatedObject)));
        return;
    }
    ...
    private void UpdateMaskVisual(Compositor _compositor)
    {
        ...
        if (AssociatedObject is not Shape and not TextBlock)
        {
            HandleCornerRadiusClip(AssociatedObject, _compositor, _maskVisual);
        }
    }
    ...
    private void UpdateMaskBrush(Compositor _compositor)
    {
        ...
        if (AssociatedObject is Shape shape)
        {
            _maskBrush.Mask = shape.GetAlphaMask();
        }

        else if (AssociatedObject is TextBlock block)
        {
    ...
    private void DisposeResources()
    {
        ...
        ElementCompositionPreview.SetElementChildVisual(AssociatedObject, null);
    ...
    {
        ElementCompositionPreview.SetElementChildVisual(AssociatedObject, _maskVisual);
    ...
    private void RemoveColorListeners()
    {
        ...
        {
            AssociatedObject.UnregisterPropertyChangedCallback(_colorSourceProperty, _colorListenerToken.Value);
        }
    }
    ...
    private void AddColorListeners()
    {
        ...
        {
            _colorListenerToken = AssociatedObject.RegisterPropertyChangedCallback(_colorSourceProperty, OnColorSourceUpdated);
        }
    }
